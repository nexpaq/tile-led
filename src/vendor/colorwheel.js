...
Raphael.colorwheel = function(target, color_wheel_size, no_segments){
  ...
  function create(target, color_wheel_size){
    ...
    bs_square.cursor = cursor_create(0);
    events_setup();
  ...
  function cursor_create(size){
    var set = canvas.set().push(
        canvas.circle(0, 0, size).attr({"stroke-width":4, stroke:"white"})
    );
  ...
  function update_color(dont_replace_input_value){
    var x = bs_square.cursor.items[0].attr("cx"),
        ...
        hsb = {
          b: 1, //changed 1-(y-sdim.y)/sdim.l to 1
          s: 1,   //changed (x-sdim.x)/sdim.l to 1
          h: hue()
        };
  ...
  function set_hue_cursor(mixed_args){
    ...
    var x = Math.cos(radians(d)) * (center-tri_size);
    var y = Math.sin(radians(d)) * (center-tri_size);
  ...
  function create_bs_square(){
    ...
    bs_square.h = canvas.rect.apply(canvas, box).attr({ display: "none", visibility:"hidden",
      stroke:"#EEE", opacity:0});
    bs_square.s = canvas.rect.apply(canvas, box).attr({ display: "none", visibility:"hidden",
      stroke:null,  opacity:0});
    bs_square.b = canvas.rect.apply(canvas, box).attr({ display: "none", visibility:"hidden",
      stroke:null, opacity:0});
  ...
  function copy_segment(r, d, k){
    ...
    n.attr({"stroke-width":0, fill:"hsb("+d*(1/k)+", 1, 1)"}); //changed last element(b) 0.85 to 1
